...
import { type WatchlistItem } from "@/app/(dashboard)/dashboard/actions";
...
export function WatchlistTable({ initialItems }: WatchlistTableProps) {
  ...
  const [addAssetType, setAddAssetType] = useState<"stock" | "etf">("stock");
  ...
  const stockCount = items.filter((i) => i.asset_type === "stock").length;
  ...
  // Quick Add / Search Trigger Handler
  const handleAddOrSearchTicker = (e: React.FormEvent) => {
    e.preventDefault();
    const formattedTicker = addTickerInput.trim().toUpperCase();
    if (!formattedTicker) return;

    // 1. Immediately update active selected ticker to trigger chart synchronization
    setSelectedTicker(formattedTicker);

    // 2. Check if already in tracked assets list; if not, add it dynamically
    const exists = items.some((i) => i.ticker === formattedTicker);
    if (!exists) {
      const newItem: WatchlistItem = {
        id: `custom-${Date.now()}`,
        ticker: formattedTicker,
        name: `${formattedTicker} ${addAssetType.toUpperCase()}`,
        asset_type: addAssetType,
        price: 150 + Math.random() * 100,
        change: (Math.random() - 0.4) * 8,
        change_percent: (Math.random() - 0.4) * 4,
      };
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      {/* Search & Add Ticker Sync Bar */}
      <div className="p-4 bg-neutral-900 border border-neutral-800 rounded-xl shadow-lg flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <span className="text-xs font-bold text-blue-400 uppercase font-mono block">Chart & Radar Sync</span>
          <h3 className="text-sm font-bold text-white">Search & Select Active Asset</h3>
        </div>

        <form onSubmit={handleAddOrSearchTicker} className="flex items-center gap-2">
          <input
            type="text"
            value={addTickerInput}
            onChange={(e) => setAddTickerInput(e.target.value)}
            placeholder="Search / Add Ticker (e.g. KO)"
            className="bg-neutral-950 border border-neutral-800 rounded-lg px-3 py-2 text-xs text-white placeholder-gray-500 uppercase focus:outline-none focus:ring-2 focus:ring-blue-500 w-44 font-mono font-bold"
            required
          />
          <select
            value={addAssetType}
            onChange={(e: any) => setAddAssetType(e.target.value)}
            className="bg-neutral-950 border border-neutral-800 rounded-lg px-2.5 py-2 text-xs text-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono"
          >
          ...
          <button
            type="submit"
            className="px-4 py-2 bg-blue-600 hover:bg-blue-500 active:scale-95 text-white font-bold text-xs rounded-lg shadow-md shadow-blue-600/30 transition-all shrink-0"
          >
            + Add & Sync Chart
          </button>
        </form>
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3 bg-neutral-900/60 border border-neutral-800/80 rounded-xl p-3.5 px-5">
          ...
          <span className="text-[11px] text-gray-400">
            Active Selected: <strong className="text-white font-mono">{selectedTicker}</strong>
          </span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center justify-between">
          ...
          <div className="relative w-full sm:w-80 md:w-96">
            ...
            <input
              type="text"
              placeholder="Filter Table by Ticker or Name..."
              value={searchTerm}
              ...
              filteredItems.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-right font-medium text-white font-mono">
                      ${item.price.toFixed(2)}
                    </td>
                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-right font-medium font-mono">
                      <span
